...
import { CatalogoModel, PaginacionModel, ProcesoModel, ParametrosModel } from 'src/app/common/models/Index';
...
@Component({
  selector: 'app-rectificado-listar',
  templateUrl: './rectificado-listar.component.html',
  styleUrls: ['./rectificado-listar.component.css']
})
export class RectificadoListarComponent implements OnInit {
  ...
  consultarParemtros() {
    this.parametroService.consultarParametrosPorEntidad('PROCESO').subscribe(response => {
      ...
      this.obtenerTipoProceso(this.tipoProcesos, 'rectificado');
    }, error => {
      console.log(error)
